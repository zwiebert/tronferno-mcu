...
process_parmTimer(clpar p[], int len) {
  ...
  bool need_reload_td, need_save_td;
  {
    bool f_modified = !f_modify || td_is_astro(&tda) || f_disableAstro || td_is_daily(&tda) || f_disableDaily || td_is_weekly(&tda) || f_disableWeekly || GET_BIT(fpr0_mask, flag_Random) || GET_BIT(fpr0_mask, flag_SunAuto);
