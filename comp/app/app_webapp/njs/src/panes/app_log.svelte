...
<div class="flex-row">
  Filters:
  <label class="bg-red-400">MQTT-RX
  ...
  <label class="bg-green-400">RF-RX
    <input type="checkbox" bind:checked={showRc} /></label>
  <label class="bg-blue-400">RF-TX
  ...
  {#each $AppLog as msg}
    {#if 'log' in msg}
      {#if 'mqtt_client' === msg.log.tag && msg.log.txt.startsWith("receive")}
        {#if showMqttRx}
          <span class="text-red-400">{msg.date.toLocaleTimeString() + ': ' + msg.log.tag + ': ' + msg.log.txt}<br /></span>
        {/if}
    ...
    {:else if showRc && 'rc' in msg}
      <span
        class="text-green-400">{msg.date.toLocaleTimeString() + ': ' + 'rx: ' + msg.rc.type + ': ' + msg.rc.a
    ...
    {:else if showSc && 'sc' in msg}
      <span
        class="text-blue-400">{msg.date.toLocaleTimeString() + ': ' + 'tx: ' + msg.sc.type + ': ' + msg.sc.a 
