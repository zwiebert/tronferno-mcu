<script>
  ...
  function genText_mqtt(gmu) {
    let txt =
      ...
      "# Paste it into a raw definition window in FHEM-Web\n" +
      `# and press button "execute-commands"\n\n`;
  ...
  function genText_module(gmu) {
    let txt =
      "# Configuration for Tronferno FHEM-module\n" + //
      "# Paste it into a raw definition window in FHEM-Web\n" +
      `# and press button "execute-commands"\n\n`;
    for (let g = 0; g < gmu.length; ++g) {
      if (g === 0 || gmu[g])
...
<div class="main-area">
  ...
  <div class="area">
    ...
    ><br />
    <textarea value={moduleText} style="font-size:6pt" cols={56} rows={16} disabled={true} />
  </div>
</div>
